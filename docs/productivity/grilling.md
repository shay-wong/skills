...
Into this skill. Round-based questioning shipped briefly as a separate skill, then moved into `grilling` itself, so everything built on the discipline (`grill-me`, `grill-with-docs`, `triage`, `wayfinder`) got it at once. There is no `batch-grill-me` to install, and no separate sequential skill either; the `CLAUDE.md` line above is the way back to one-at-a-time.
...
**How do `grill-me` and `grill-with-docs` reach this Skill?**
They call the Skill tool when the harness exposes one. In Codex they can locate `grilling` in the available Skills catalog and read its complete `SKILL.md`, the same composition pattern used by the `review` orchestrator. Both routes require `grilling` to be installed.
...
`grilling` is a **primitive**, not a step you schedule: the single source of truth for the interview technique. [grill-me](https://aihero.dev/skills-grill-me) and [grill-with-docs](https://aihero.dev/skills-grill-with-docs) are its two user-invoked front doors. `grill-with-docs` is where the main build chain begins, ahead of [to-spec](https://aihero.dev/skills-to-spec). [wayfinder](https://aihero.dev/skills-wayfinder), [triage](https://aihero.dev/skills-triage), and [improve-codebase-architecture](https://aihero.dev/skills-improve-codebase-architecture) apply the same discipline inside their own workflows. When you are unsure which entry point fits, [ask-me](https://aihero.dev/skills-ask-me) routes you.
