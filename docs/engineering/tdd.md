...
| Config, wiring, glue, type annotations, docs or metadata | Skip TDD and run the smallest relevant verification |

The risk gate closes the old gap where an implementation request could trigger a test that merely restated the code. Coverage follows the failure boundary: a focused regression test can be enough for a narrow fix, while a cross-system change may need several distinct test types.
...
[to-spec](https://aihero.dev/skills-to-spec) agrees the test seams up front, [implement](https://aihero.dev/skills-implement) drives `tdd` per ticket, and [review](https://aihero.dev/skills-review) uses [code-review](https://aihero.dev/skills-code-review) as its primary to check afterwards that only the agreed seams were used. That primary also owns the refactoring `tdd` no longer does. Its other neighbour is [codebase-design](https://aihero.dev/skills-codebase-design), the shared source of the seam and deep-module vocabulary `tdd` speaks. You can also reach for it on its own, whenever there is a concrete behaviour to build and no full spec in play. When you are unsure which skill fits your situation, [ask-me](https://aihero.dev/skills-ask-me) routes you.
