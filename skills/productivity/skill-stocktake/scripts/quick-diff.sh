...
PROJECT_SKILLS_DIR="${SKILL_STOCKTAKE_PROJECT_DIR:-${2:-}}"
...
[[ -n "$PROJECT_SKILLS_DIR" && -d "$PROJECT_SKILLS_DIR" ]] && process_dir "$PROJECT_SKILLS_DIR"

if [[ $i -eq 0 ]]; then
  echo "[]"
else
  jq -s 'unique_by(.path)' "$tmpdir"/*.json
fi
