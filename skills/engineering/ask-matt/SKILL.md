...
3. **Branch: is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally (one red-green slice at a time), then closes out by running **`/code-review`**, a two-axis review (Standards + Spec) of the diff, before committing. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec. Reach for **`/review`** when you want a general review entrypoint that keeps `/code-review` as the normal primary and adds distinct installed reviewers, including Ponytail when available.
...
- **`/review`** is the personal review orchestrator. It discovers installed review Skills at invocation time, selects one primary, runs only distinct complementary scopes, and synthesizes one evidence-backed verdict. Use `/code-review` directly when you only need its Standards and Spec axes.
