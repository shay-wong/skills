...
| 本地改动 | `review`、`commit` 与 `worktree-clean` 成为 promoted engineering Skill；`review` 保留为轻量统筹入口，优先复用当前 harness 的可用 Skill 清单，仅在清单缺失或不完整时扫描本地审查 Skill，并在 Ponytail 可用时把 `ponytail:ponytail-review` 作为最后的建议性审查；开发、测试、证据规则和 Git 冲突规则继续并入 Matt `implement`、`tdd`、`code-review` 与 `resolving-merge-conflicts`；`agent-rules`、standards 和 development wrapper 删除 |
